...
The resulting histogram does not provide any additional insights, therefore we will discard it and use the original data in the following discussion. Since the data we have chosen is concerning only the top 500 valuable players, the dataset is not normally distributed but right-skewed. After the log transformation, the dataset remains right-skewed and it's less informative for readers to understand values intuitively. Therefore, we decide to adhere to the original model and reject the log transformation.
...
Then, we perform backward selection of all the given variables.
...
```{r r-squared}
glance(linear_prediction)$r.squared
glance(selected_model)$r.squared
```

Through the model selection based on AIC, we can see that the variables "age", "own_goals", "yellow_cards", "red_cards", "substituted_on", and "substituted_off" are eliminated. The AIC is reduced compared to the previous dataset (4513.152 to 4504.674), which can be interpreted as the increased likelihood of the model. When we take a look at the r-squared value before and after selection, it stays on a similar level.
...
market_value = 15.9 - 16.5 * position_newForward + 6.77 * position_newGoalkeeper + 3.71 * position_newMidfielder + 0.406 * matches + 2.02 * goals + 0.885 * assists + 3.66 * age_range21-25 + 9.07 * age_range26-30 - 0.922 age_range30 and above + 0.937 * position_newForward:goals - 1.45 * position_newMidfielder:goals + 2.74 * position_newForward:assists + 23.4 * position_newGoalkeeper:assists - 0.366 * position_newMidfielder:assists
...
For Goalkeepers : 
market_value = 22.67 + 0.406 * matches + 2.02 * goals + 24.285 * assists + 3.66 * age_range21-25 + 9.07 * age_range26-30 - 0.922 age_range30 and above
...
For Midfielders : 
market_value = 19.61 + 0.406 * matches + 0.57 * goals + 0.519 * assists + 3.66 * age_range21-25 + 9.07 * age_range26-30 - 0.922 age_range30 and above
...
For Forwards : 
market_value = -0.6 + 0.406 * matches + 2.957 * goals + 3.659 * assists + 3.66 * age_range21-25 + 9.07 * age_range26-30 - 0.922 age_range30 and above
